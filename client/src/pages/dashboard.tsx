    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-12 w-full">
        ...
        {monitors && monitors.length > 0 && (
          <div className="flex flex-wrap gap-2.5 z-10">
            <Button
              ...
              className={`rounded-full transition-all duration-300 font-semibold px-5 hover-lift active-press ${
                filterMonitor === 'all'
                  ? 'glass-card-premium text-primary-foreground shadow-[0_0_20px_hsl(var(--primary)/0.3)] border-transparent animate-pulse-glow'
                  : 'glass-card-subtle border border-white/10 hover:text-foreground hover:border-white/20'
              }`}
            >
              All Items
            </Button>
      ...
      {itemsLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <Card key={i} className="p-6 space-y-5 glass-card-premium animate-pulse border-white/5">
              <Skeleton className="h-7 w-3/4 rounded-xl glass-card-subtle" />
              <Skeleton className="h-20 w-full rounded-xl glass-card-subtle" />
              <div className="flex justify-between items-center pt-4 border-t border-white/5">
                <Skeleton className="h-5 w-1/3 rounded-md glass-card-subtle" />
                <Skeleton className="h-9 w-10 rounded-xl glass-card-subtle" />
              </div>
            </Card>
          ))}
        </div>
        ...
        >
          ...
          <h2 className="text-3xl font-display font-bold mb-3 text-gradient-sapphire">No items found</h2>
            ...
            {filteredItems.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Card className="h-full flex flex-col p-7 glass-card-premium hover-lift relative overflow-hidden rounded-2xl group-hover:shadow-[0_0_30px_hsl(var(--primary)/0.1)]">
                      ...
                      <div className="absolute top-0 right-0 p-5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-x-2 group-hover:translate-x-0">
                         <ExternalLink className="w-5 h-5 text-primary drop-shadow-[0_0_8px_hsl(var(--primary)/0.5)]" />
                      </div>

                      <div className="inline-flex items-center gap-2 mb-4 text-xs font-bold text-gradient-primary uppercase tracking-widest glass-card-subtle w-fit px-3 py-1.5 rounded-full border border-primary/20 shadow-[0_0_10px_hsl(var(--primary)/0.05)] animate-shimmer">
                        <Tag className="w-3.5 h-3.5" />
                        {monitor?.name || 'Unknown Monitor'}
                      </div>

                      <h3 className="text-xl font-bold font-display text-foreground line-clamp-2 leading-tight mb-3 group-hover:text-gradient-primary transition-colors duration-300 relative z-10">
                        {item.title}
                      </h3>
